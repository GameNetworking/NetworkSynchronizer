		...
		if (is_default_A && is_default_B) {
			...
			switch (info.data_type) {
				...
				case DataBuffer::DATA_TYPE_UNIT_REAL:
					...
					break;
				case DataBuffer::DATA_TYPE_VECTOR2:
					are_equals = NS::SceneSynchronizerBase::compare(p_buffer_A.read_vector2(info.compression_level), p_buffer_B.read_vector2(info.compression_level), info.comparison_floating_point_precision);
					break;
				case DataBuffer::DATA_TYPE_NORMALIZED_VECTOR2:
					are_equals = NS::SceneSynchronizerBase::compare(p_buffer_A.read_normalized_vector2(info.compression_level), p_buffer_B.read_normalized_vector2(info.compression_level), info.comparison_floating_point_precision);
					break;
				case DataBuffer::DATA_TYPE_VECTOR3:
					are_equals = NS::SceneSynchronizerBase::compare(p_buffer_A.read_vector3(info.compression_level), p_buffer_B.read_vector3(info.compression_level), info.comparison_floating_point_precision);
					break;
				case DataBuffer::DATA_TYPE_NORMALIZED_VECTOR3:
					are_equals = NS::SceneSynchronizerBase::compare(p_buffer_A.read_normalized_vector3(info.compression_level), p_buffer_B.read_normalized_vector3(info.compression_level), info.comparison_floating_point_precision);
					break;
				case DataBuffer::DATA_TYPE_BITS:
					CRASH_NOW_MSG("Not supported.");
					break;
				case DataBuffer::DATA_TYPE_VARIANT:
					are_equals = NS::SceneSynchronizerBase::compare(p_buffer_A.read_variant(), p_buffer_B.read_variant(), info.comparison_floating_point_precision);
					break;
			};
		}
