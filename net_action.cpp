...
void net_action::decode_net_action(
	...
	while (p_data_buffer.get_bit_offset() < p_data_buffer.total_size()) {
		...
		if (uses_node_id) {
			...
			if (node == nullptr) {
				NET_DEBUG_ERR("The received action data contains a node path which is unknown: `" + node_path + "`");
				continue;
			}
			node_data = synchronizer->find_node_data(node);
			if (node_data == nullptr) {
				NET_DEBUG_ERR("The received action data contains a node which is not registered on this peer. NodePath: `" + node_path + "`");
				continue;
			}
		}
